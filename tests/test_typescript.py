...
def test_comment_only_catch_is_still_reported():
    ...
    assert [f.rule_id for f in TypeScriptRulePack().evaluate(parsed)] == ["TS-COR-001"]
...
def test_undeclared_bare_import_is_reported(project):
    ...
    pkg_findings = [f for f in payload["findings"] if f["rule_id"] == "TS-PKG-001"]
...
def test_workspaces_manifest_skips_drift_analysis(project):
    ...
    assert not [f for f in payload["findings"] if f["rule_id"] == "TS-PKG-001"]
...
def test_invalid_package_json_is_an_error_finding(project):
    ...
    assert [f["rule_id"] for f in payload["findings"] if f["rule_id"].startswith("TS-PKG")] == [
        "TS-PKG-002"
    ]
...
def test_missing_manifest_produces_no_package_findings(project):
    ...
    assert not [f for f in payload["findings"] if f["rule_id"].startswith("TS-PKG")]
...
def test_nested_manifest_drift_is_discovered(project):
    ...
    result = CliRunner().invoke(main, [str(root), "-f", "json"])
    payload = json.loads(result.output)
    drift = [f for f in payload["findings"] if f["rule_id"] == "TS-PKG-001"]
    ...
    root = project(
        {
            ...
            "app/src/main.ts": ("import express from 'express';\n" "import { z } from 'zod';\n"),
        }
    )
...
def test_invalid_nested_manifest_reports_and_skips_drift(project):
    ...
    result = CliRunner().invoke(main, [str(root), "-f", "json"])
    payload = json.loads(result.output)
    rules = [f["rule_id"] for f in payload["findings"] if f["rule_id"].startswith("TS-PKG")]

    assert rules == ["TS-PKG-002"]
    invalid = [f for f in payload["findings"] if f["rule_id"] == "TS-PKG-002"][0]
...
def test_keyword_glued_strings_are_still_strings():
    # Round 2, B1: the JSX-apostrophe rule must not swallow `return'x'`,
    # `case'a':` or `typeof'a'` (valid, common in minified bundles).
    source = (
        "function f(){return'trie'}\nswitch(x){case'dijkstra':break}\n"
        "if(typeof'a'){}\n<p>Don't</p>\n"
    )
    blanked, complete = _strip_ts_comments_and_strings(source)
    assert complete
    assert "trie" not in blanked and "dijkstra" not in blanked
    assert "Don't" in blanked and "break" in blanked
